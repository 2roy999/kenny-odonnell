...
exports.Gsheet = class {
  ...
  readData(range) {
    ...
    return Promise.fromCallback(cb => sheets.spreadsheets.values.get({
  ...
  writeData(range, data) {
    ...
    return Promise.fromCallback(cb => sheets.spreadsheets.values.update({
      spreadsheetId: this.spreadsheetId,
      range,
  ...
  createSpreadsheet(sheetName, { skipIfExists } = {}) {
    ...
    return Promise.fromCallback(cb => sheets.spreadsheets.get({
      spreadsheetId: this.spreadsheetId
    }, cb))
      .then(response => {
        if (response.status === 200) {
